...
class NavBar(Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.parent = parent
        self.config(
                   background=BUTTON_BACKGROUND , cursor="hand1"
                   )
...
class ServerFrame(Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.currentline = 1
        self.header_frame = Frame(self, background=WINDOW_BACKGROUND)
        
        # TODO Ask for confirm deletion
        
        # Global fs_basepath
        self.fs_basepath_var = StringVar()
        self.fs_basepath_label = Label(self.header_frame, text="fs_basepath: ", font=font, background=WINDOW_BACKGROUND)
        ...
        self.fs_basepath_entry.bind(sequence='<KeyRelease>', func=self.updateconfig)  # TODO
        self.fs_basepath_label.grid(row=0, column=0, sticky=N + W)
        self.fs_basepath_entry.grid(row=0, column=1, sticky=N + W + E)
        self.fs_basepath_browse.grid(row=0, column=2, sticky=N + E)
        
        # Global fs_homepath
        self.fs_homepath_var = StringVar()
        self.fs_homepath_label = Label(self.header_frame, text="fs_homepath: ", font=font, background=WINDOW_BACKGROUND)
        ...
        self.fs_homepath_entry.bind(sequence='<KeyRelease>', func=self.updateconfig)  # TODO
        self.fs_homepath_label.grid(row=1, column=0, sticky=N + W)
        self.fs_homepath_entry.grid(row=1, column=1, sticky=N + W + E)
        self.fs_homepath_browse.grid(row=1, column=2, sticky=N + E)
        
        # Global ETPath
        self.etpath_var = StringVar()
        self.etpath_label = Label(self.header_frame, text="ET: ", font=font, background=WINDOW_BACKGROUND)
        ...
        self.etpath_entry.bind(sequence='<KeyRelease>', func=self.updateconfig)  # TODO
        self.etpath_label.grid(row=2, column=0, sticky=N + W)
        self.etpath_entry.grid(row=2, column=1, sticky=N + W + E)
        self.etpath_browse.grid(row=2, column=2, sticky=N + E)
        
        # Global Parameters
        self.parameters_var = StringVar()
        self.parameters_label = Label(self.header_frame, text="Parameters: ", font=font, background=WINDOW_BACKGROUND)
        ...
        self.parameters_entry.bind(sequence='<KeyRelease>', func=self.updateconfig)  # TODO
        ...
        # Server List Frame
        self.serverlist_frame = Frame(self, background=WINDOW_BACKGROUND)
        
        # Server List Titles
        self.servers_label = Label(self.serverlist_frame, text="Title", font=font, background=WINDOW_BACKGROUND)
        ...
        # Server Map
        self.servermap_label = Label(self.serverlist_frame, text="Map", font=font, background=WINDOW_BACKGROUND)
        ...
        # Server Players
        self.serverplayers_label = Label(self.serverlist_frame, text="Players", font=font, background=WINDOW_BACKGROUND)
        ...
        # Server Ping
        self.serverping_label = Label(self.serverlist_frame, text="Ping", font=font, background=WINDOW_BACKGROUND)
        ...
        # Server Data Frame
        self.serverdata_frame = Frame(self, background=WINDOW_BACKGROUND)
        
        # Server title
        self.servertitle_var = StringVar()
        self.servertitle_label = Label(self.serverdata_frame, text="Title: ", font=font, background=WINDOW_BACKGROUND)
        ...
        # Server Password
        # TODO Only show if server is password protected
        self.serverpassword_var = StringVar()
        self.serverpassword_label = Label(self.serverdata_frame, text="Password: ", font=font, background=WINDOW_BACKGROUND)
        ...
        self.serveraddress_label = Label(self.serverdata_frame, text="Address: ", font=font, background=WINDOW_BACKGROUND)
        ...
        self.serverparams_label = Label(self.serverdata_frame, text="Parameters: ", font=font, background=WINDOW_BACKGROUND)
        ...
        self.serverparams_label.grid(row=3, column=0, sticky=N + W)
        self.serverparams_entry.grid(row=3, column=1, sticky=N + W + E)
        
        # Server fs_basepath
        self.serverfs_basepath_var = StringVar()
        self.serverfs_basepath_label = Label(self.serverdata_frame, text="fs_basepath: ", font=font, background=WINDOW_BACKGROUND)
        ...
        self.serverfs_homepath_label = Label(self.serverdata_frame, text="fs_homepath: ", font=font, background=WINDOW_BACKGROUND)
        ...
        self.serveretpath_label = Label(self.serverdata_frame, text="ET: ", font=font, background=WINDOW_BACKGROUND)
        ...
        self.serveretpath_entry.bind(sequence='<KeyRelease>', func=self.updateserver)  # TODO
        self.serveretpath_label.grid(row=6, column=0, sticky=N + W)
        self.serveretpath_entry.grid(row=6, column=1, sticky=N + W + E)
        self.serveretpath_browse.grid(row=6, column=2, sticky=N + E)
    ...
    def removeserver(self):
        ...
        if not self.parent.serverdata.Servers[self.getselection()[0]]:
            print("Error updating server %d" % self.getselection()[0])
            return
    ...
    def joinserver(self):
        ...
        if not Server:
            print("Error updating server %d" % self.getselection()[0])
            return
        print("Joining server %s" % Server['title'])
        ...
        if not isfile(etpath):
            print("ET Executable is not valid")
            return
        if not isdir(fs_basepath):
            print("fs_basepath is not valid")
            return
        if not isdir(fs_homepath):
            print("fs_homepath is not valid")
            return
    ...
    def serverstatus(self, specificserver=None):
        ...
        if not Server:
            print("Error getting server playerlist %d" % selectid[0])
            return
        print("Getting serverstatus for %d (%s)" % (selectid[0], Server['title']))
        self.currentline = 0
        ...
        if specificserver == None:
            ...
            self.text.config(state=DISABLED)
            if "sv_hostname" in Server['cvar']: self.insertline(cleanstr(Server['cvar']['sv_hostname']).center(PLAYER_TOTAL_LENGTH))
            if "mapname" in Server['cvar']: self.insertline(Server['cvar']['mapname'].center(PLAYER_TOTAL_LENGTH))
            self.insertline((str(Server['ping']) + 'ms').center(PLAYER_TOTAL_LENGTH))
            self.insertline('')
        ...
        if specificserver == None:
            halflen = int((PLAYER_TOTAL_LENGTH - 3) / 2)
            self.insertline('')
            self.insertline('')
    ...
    def selectserver(self, e):
        ...
        Server = self.parent.serverdata.Servers[selectid[0]]
        if not Server:
            print("Error updating server %d" % selectid[0])
            return
        print("Selecting server %s" % Server['title'])
...
class Window(Frame):
    ...
    def load_config(self):
        global WINDOW_BACKGROUND, WINDOW_BORDER, ENTRY_BACKGROUND, ENTRY_FOREGROUND, LIST_SELECT_FORE, LIST_SELECT_BACK, BUTTON_BACKGROUND, BUTTON_FOREGROUND, A_BUTTON_BACKGROUND, A_BUTTON_FOREGROUND
        self.Config = {
        ...
        if not isfile(join(getcwd() , 'wolfstarter.json')):
            ...
            if jsondata: self.Config = jsondata
            for key in ['servers', 'WINDOW_BACKGROUND', 'WINDOW_BORDER', 'ENTRY_BACKGROUND', 'ENTRY_FOREGROUND',
                        ...
                        'A_BUTTON_BACKGROUND', 'A_BUTTON_FOREGROUND']:
    ...
    def save_config(self):
        ...
        json.dump(self.Config, jsonfile, skipkeys=True, allow_nan=False, sort_keys=True, indent=4)
    ...
    def openfile(self):
        ...
        if not fname or not isfile(fname):
            ...
            print("Invalid Servers file was not found %s" % fname)
        ...
        self.serverdata        = ServerData()
        self.serverdata.load_serverfile(self.Config['servers'])
        print("Loaded Servers file: %s" % self.Config['servers'])
    ...
    def saveasfile(self):
        ...
        self.Config['servers'] = fname
        self.serverdata.save_serverfile(self.Config['servers'])
        print("Saved servers file %s" % self.Config['servers'])
    def quit(self):
        self.save_config()
        self.parent.destroy()
